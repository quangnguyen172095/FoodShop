...
public class CreateOrderServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action != null) {
            ...
            List<OrderDetails> details = detailsDAO.SearchByID(orderID);
...
//        Admin lay tu login
            ...
            Timestamp orderDate = Timestamp.valueOf(dateTime.atZone(ZoneId.systemDefault()).toLocalDateTime()); // Convert LocalDateTime to Timestamp

            String[] products = request.getParameterValues("product[]");
            String[] quantities = request.getParameterValues("quantity[]");
            for (int i = 0; i < products.length; i++) {
                detailsList.add(new OrderDetails(null, productsDAO.SearchByID(Integer.parseInt(products[i])),
            ...
            Orders orders = ordersDAO.SearchByID(ordersDAO.insertOrders(new Orders(0, customers, orderstatus, orderDate,
                    payment, csaddress, freight, admin, createdOn, transactionstatus)));
            for (OrderDetails od : detailsList) {
                od.setOrders(orders);
            ...
            if (ordersDAO.updateOrder(orderstatus, payment, csaddress, createdOn, transactionstatus, orderID)) {
            }
        }
