...
public class AddCart extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action == null || action.trim().equals("")) {
            ...
            String totalPrice = "<strong>" + decimalFormat.format(order.getTotalPrice()) + " VNĐ</strong>";
            String cartList = "";
            for (OrderDetail item : order.getOrderDetails()) {
                float unitPrice = item.getPrice() - item.getPrice() * item.getDiscount();
                float totalPricePerItem = unitPrice * item.getQuantity();

                String formattedUnitPrice = decimalFormat.format(unitPrice); // Định dạng giá unitPrice
                String formattedTotalPricePerItem = decimalFormat.format(totalPricePerItem); // Định dạng giá totalPricePerItem
